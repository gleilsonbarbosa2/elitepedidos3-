/*
  ...
  1. Changes
    ...
    - Ensure admin has access to all areas of the site
...
-- Update all operators to have the necessary permissions
UPDATE pdv_operators
...
WHERE permissions->>'can_use_scale' IS NULL;
...
SET permissions = jsonb_set(
  COALESCE(permissions, '{}'::jsonb),
  '{can_manage_settings}',
  CASE WHEN code = 'ADMIN' OR code = 'admin' THEN 'true'::jsonb ELSE 'false'::jsonb END
)
WHERE permissions->>'can_manage_settings' IS NULL;

-- Update the default permissions for new operators
ALTER TABLE pdv_operators
ALTER COLUMN permissions SET DEFAULT '{"can_discount": true, "can_cancel": true, "can_manage_products": false, "can_view_sales": true, "can_view_cash_register": true, "can_view_products": true, "can_view_orders": true, "can_view_reports": true, "can_view_sales_report": true, "can_view_cash_report": true, "can_view_operators": false, "can_view_attendance": true, "can_use_scale": true, "can_manage_settings": false}';